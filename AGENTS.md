...
> 2026-09-22 가격 진단 후속: 계산과 같은 H/L/C 검증·수치 오차 기준을 공유하며 수집 단계별 고정 위반 유형/과거·마지막 행 여부만 배치 단위로 집계한다. 새 진단에는 종목·날짜·가격·건수·순서를 넣지 않는다. 가격 수정·행 제거·임계치·계산·거래·알림·실패 종료 정책은 변경하지 않는다. 현재 계약은 `docs/price-diagnostics.md`를 따른다.

> 2026-09-22 관리·입력 후속: 관리 옵션은 `position_cli.py`에서 설정/시세 import 전에 처리한다. 로컬 전용 변경과 미확정 Drive 후보는 비공개 저널에 보존하고 일반 동기화 덮어쓰기를 막는다. CSV/XLSX 전체 검증, Telegram 본문 성공 확인, 로컬 실패 종료와 기존 예약 유지의 현재 계약은 `docs/management-and-local-outcomes.md`를 우선한다. 관리 경로는 `.env`를 읽지 않으며 Drive 모드는 현재 프로세스 환경의 두 Secret을 명시해야 한다. 실제 운영 명령·새 전략·원격 CAS 완료를 뜻하지 않는다.

> 2026-09-22 상태 충돌 후속 구현: `state_lock.py`의 경로별 재진입 가능한 OS 잠금으로 로컬 상태 변경과 GHA pull→작업→push를 직렬화한다. Drive는 성공한 pull의 MD5와 업로드 직전 원격 MD5를 비교하고 충돌·확인 실패 시 쓰지 않는다. 사전 검사와 업로드 사이 경쟁은 남으므로 원자적 CAS로 부르지 않는다. 현재 계약·잠금 파일 취급·검증 경로와 R06/R07 잔여는 `docs/state-conflicts.md`를 우선한다. 일반 오프라인 runner의 프로세스 차단은 그대로이며, 실제 잠금 검사는 별도 `scripts/run_state_lock_tests.py`의 합성 자식만 사용한다.

> 2026-09-22 시장 분류 후속 구현: `symbol_market.get_trading_market()`으로 거래시장과 ETF 자산유형을 분리했다. 한국 ETF의 알림 게이트는 한국 시장을 사용하되 기존 ETF ATR 배수는 유지한다. 시장 불명 숫자 심볼은 설정 오류이며, 크립토 검색은 정확 일치 우선·모호한 숫자 변형 보류다. 현재 계약과 한계는 `docs/market-routing.md`를 참고한다. 시장 시간·실행 창·전략 수치는 변경하지 않았다.

> 2026-09-22 시장 날짜 후속 구현: 수집 조회 범위·당일 트리거·알림 중복 키는 `market_dates.py`의 시장 날짜를 사용한다(KR 서울, 미국 뉴욕, 크립토 UTC). 최신 가격 보완은 같은 메타데이터 응답의 유효한 가격과 시각을 요구한다. 날짜 없는 가격으로 오늘 봉을 만들지 않는다. 기존 알림 상태는 일괄 변환하지 않는다. 현재 계약과 한계는 `docs/market-dates.md`를 우선하며, 전략 임계값·시장 활성 시간·실행 스케줄은 유지한다.
