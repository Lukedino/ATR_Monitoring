...
## 2026-09-21 — 종가 요약: 시장별 범위 복구 + 이탈·근접 전 종목 나열

**회귀**: "KR 종가 요약" 에 미국 종목이 섞여 전 종목 수로 나갔다. 09-15 `4ba6af5` 에서
stop_check 을 창과 분리해 전 종목으로 돌리면서, 요약이 그 전 종목 결과를 그대로 받았다 — 그 전에는
`job_stop_check(_WINDOW_SYMBOLS[market])` 결과를 받아 자연히 시장별이었고, 분리 후 `_WINDOW_SYMBOLS` 는 사용처 없이 남아 있었다.
- `_send_daily_brief` 가 `_BRIEF_SCOPE[window.market]` 의 제목·종목으로 Chandelier·시세·Stop 갱신을 골라 센다. KR = .KS/.KQ → "KR 종가 요약", US = 그 외(ETF 포함) + 크립토 → **"US/크립토 종가 요약"**.
- 크립토는 전용 종가 창이 없어 09-15 이전에는 어느 요약에도 없었다. 소유자 요청으로 주간 리포트와 같은 묶음(미국/크립토)에 붙였다 — 평일 us_close 에만 나가므로 주말에는 크립토 요약이 없다. 보유가 정확히 한 요약에만 들어가는지, 종가 창마다 범위가 정의돼 있는지를 테스트로 고정.
- 기준일도 그 시장 종목끼리 다시 구한다 — 전 종목 최빈값은 미국 거래일일 수 있다. `StopCheckResult.data_date` 제거, `updated_count` → `updated_symbols`.

**소유자 결정**: 이탈·근접 종목 이름을 5개로 접던 상한(`BRIEF_NAME_LIMIT`) 제거 — "… 외 28개" 로는 어느 종목을 손봐야 하는지 알 수 없다.
여유 종목은 계속 개수만. 길어지면 `send_long_message` 가 나눠 보내고, 이제 한 조각이라도 실패하면 `False` 를 돌려줘 창이 완료로 적히지 않는다. 테스트 157 → 164.

## 2026-09-20 — 등록 Stop 이탈 알림 추가 (소유자 결정)

예전 트리거 4 는 `0 < dist <= 5%`(근접)만 봤다. 종가가 등록 Stop **아래**면 아무것도 붙지 않아, 전날 "근접" 을 받은 종목이
다음 날 -5% 미만 하락으로 Stop 을 깨면 이탈 당일이 무음이었다(급락 -5%·갭 -3% 트리거에 걸릴 때만 알림).
- `STOP BREACH` — 종가 ≤ 등록 Stop. 안내: 증권사 지정가 체결 여부 확인, 미체결이면 손절 실행. 날짜가 바뀌면 다시 알리므로 Stop 아래에 머무는 동안 하루 한 번 상기된다.
- `STOP MISMATCH` — 등록 Stop 이 현재가의 1.5배 초과(`STOP_MISMATCH_RATIO`). 액면분할 뒤 조정 시세와 옛 Stop 처럼 단위가 어긋난 경우로, 이탈이 아니라 **재등록 안내**를 보낸다. 이 상태에서는 Stop 갱신(new > current)도 일어나지 않는다.
- 중복 판정(`stop_manager._types`)이 `STOP` 한 단어로 묶던 것을 두 단어(`STOP NEAR`·`STOP BREACH`·`STOP MISMATCH`)로 나눠, 같은 날 근접 → 이탈 전환을 다시 알린다.
- 남은 것: 종가 요약·주간 리포트의 "이탈/근접" 표기는 아직 매번 새로 계산한 Chandelier 기준이다(등록 Stop 미반영). 테스트 146 → 157.

## 2026-09-20 — "실패했는데 초록색" 경로 차단

2026-09-19 심층 검토에서 손절 감시의 실패가 로그 한 줄 + 종료코드 0 으로 끝나는 경로가 여러 겹 확인됐다.
디스패처는 "실행이 시작됐다" 만 보므로 이런 실패는 밖에서 구별되지 않았다.

| 문제 | 수정 |
|---|---|
| `atr_calculator.py` 가 선언 없이 `logger.warning` 을 불러 KR 가격제한폭(±32%) 가드가 `NameError` — 코스닥은 `auto_adjust=False` 라 액면분할·권리락 당일에 정확히 이 분기를 탄다 | 모듈 로거 선언 |
| 종목 루프에 종목 단위 `try` 가 없어 한 종목의 예외가 뒤 종목 전부를 멈추고, 그 예외는 호출자에서 로그 한 줄로 삼켜짐 | 종목별 격리. 실패 종목 수를 모아 실행 끝에 알림 |
| 텔레그램 전송 반환값을 안 보고 `mark_trigger_sent` — 전송 실패 뒤 텔레그램이 회복돼도 "중복" 이라며 건너뜀 | 전송 성공 시에만 기록 (`job_stop_check`·`job_trigger_check`) |
| `update_stop` 이 저장을 먼저 해, 전송 실패 시 "지정가 갱신 필요" 알림이 영구 유실(`new == current`) | `update_stop(commit=False)` 로 판정 → 전송 성공 → 저장 |
| 차트 생성이 전송과 기록 사이에 있어 차트 버그 하나가 기록을 막음 | 차트는 기록 뒤, 실패해도 계속 |
| 종가 요약 전송 실패에도 창을 완료로 기록 | 전송 실패는 예외 → 창 미완료 → 다음 실행이 재시도 |
| 시세 수집이 전멸·반토막이어도 "트리거 없음" 으로 성공. 0종목 종가 요약까지 나감 | 수집률 80% 미만이면 실패 처리(받은 종목은 먼저 처리) |
| Drive 포트폴리오 로드 실패·빈 시트·헤더 변경 시 조용히 예시 종목/0종목으로 진행 | GitHub Actions 에서는 알림 후 중단. 빈 시트를 "로드됨" 으로 보지 않음 |
| `send_message` 에 429·5xx·네트워크 재시도 없음(알림이 몰리는 날 = 레이트리밋 나는 날) | 최대 3회, `retry_after` 존중 |
| 전송 예외를 그대로 로그에 찍음 — requests 예외 문자열에는 봇 토큰이 든 URL 이 있다 | 예외 종류와 HTTP 상태만 기록 |

문제가 있었던 실행은 상태를 저장한 뒤 소유자에게 요약 1건을 보내고 **종료코드 1** 로 끝난다.
테스트 122 → 146. 남은 것: 등록 Stop **이탈** 자체는 알림 조건이 아님(정책 결정 필요), 데드맨(디스패처가 죽으면 아무도 모름),
High/Low NaN 행이 이후 ATR 을 전부 NaN 으로 만드는 문제, 주간 리포트의 조기 return.
